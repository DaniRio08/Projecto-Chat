...
public class ConnectionDB {
	...
	public boolean connectar() {
		...
		try {
			this.conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/projecte_xat","root","1234");
	...
	public void searchUserByMail(User u) {
		...
		try {
			...
			System.out.println("Error a searchUser by mail: "+e.getMessage());
		}
	}
	...
	public void searchUserByPass(User u) {
		...
		try {
			...
			System.out.println("Error a searchUser by pass: "+e.getMessage());
		}
	}
	...
	public boolean searchUserBySession(User u) {
		...
		try {
			...
			System.out.println("Error a searchUser by session: "+e.getMessage());
		}
		return resposta;
	}
	...
	public boolean insertFriend(User u, User friend) {
		...
		try {
			...
			System.out.println("Error al ejecutar SQL para insertar un amigo: " + error.getMessage());
			resposta=false;
		}
		return resposta;
	}
	...
	public String getCountryList() {
		...
		try {
			...
			while (rs.next()) {
				...
				JSONObject json = new JSONObject(pais);
				ja.put(json);
			}
			jsonString = ja.toString();
		} catch (Exception e) {
			System.out.println("Error al recuperar los países: " + e.getMessage());
		}
		return jsonString;
	}
	...
	public String getFriends(User u) {
		...
		try {
			...
			System.out.println("Error al recuperar los amigos: "+e.getMessage());
		}
